#include "blue_v4_face_display.h"
...
#define TAG "BlueV4Face"

LV_FONT_DECLARE(OTTO_ICON_FONT);

BlueV4FaceDisplay::BlueV4FaceDisplay(esp_lcd_panel_io_handle_t panel_io,
...
void BlueV4FaceDisplay::SetupUI() {
    if (setup_ui_called_) {
    ...
    StartKeepAliveAnim();
    RefreshNow();
    ESP_LOGI(TAG, "Face UI ready (invert-safe, lv_anim keepalive)");
}

void BlueV4FaceDisplay::ShowFaceLocked(const char* emotion) {
...
void BlueV4FaceDisplay::ShowFace(const char* emotion) {
    DisplayLockGuard lock(this);
    ShowFaceLocked(emotion);
    RefreshNow();
}

void BlueV4FaceDisplay::SetEmotion(const char* emotion) {
    Display::SetEmotion(emotion);
    ShowFace(emotion);
}

void BlueV4FaceDisplay::SetStatus(const char* status) {
...
void BlueV4FaceDisplay::SetTheme(Theme* theme) {
    (void)theme;
    DisplayLockGuard lock(this);
    auto* lvgl_theme = static_cast<LvglTheme*>(current_theme_);
...
void BlueV4FaceDisplay::SetChatMessage(const char* role, const char* content) {
    (void)role;
    DisplayLockGuard lock(this);
    if (status_label_ == nullptr) {
        return;
